...
viz = [
    ...
    code("# US-made films (so 'home'=US/Canada is the home market); take the top 15 by",
         "# worldwide gross, then ORDER BY the split (foreign share) so it reads sorted.",
         "ww = con.execute('''",
         ...
         "    label_col='split_label', value_fmt=lambda v: v,",
         "    title='Hollywood films: home (US/Canada) vs abroad, by split',",
         "    subtitle='Top 15 US-made films by worldwide gross, ordered by share earned abroad. Gold = home, teal = rest of world.',",
         "    dot_color='#005F73', dot2_color='#EE9B00',",
...
viz_social = [
    ...
    code("# US-made films; top 15 by worldwide gross, then ordered by the split (foreign share).",
         "ww = con.execute('''",
         "    WITH us AS (SELECT title, release_year FROM films_genre WHERE is_us=TRUE),",
         "    top AS (SELECT w.title, w.release_year, w.domestic_gross, w.foreign_gross,",
         "                   100.0*w.foreign_gross/w.worldwide_gross AS foreign_pct",
