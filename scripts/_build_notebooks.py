...
clean = [
    ...
    code("ww_html = (Path(cfg['paths']['data_raw']) / 'bom_ww_top_lifetime.html').read_text(encoding='utf-8')",
         ...
         "ww.head()"),
    md("## Enrich with genre (TMDB)",
       "Look up every film (both boards) on TMDB by title + year. Requires",
       "`TMDB_API_KEY` in `.env` (gitignored). Responses are cached in",
       "`data/raw/tmdb/` so re-runs are offline. Builds `films_genre` and the",
       "long `film_genres_long` (one row per film x genre) used for the market split."),
    code("from src.ingest import load_env, tmdb_lookup_movie",
         "load_env('.env')",
...
viz_social = [
    ...
    code("genre = con.execute('''",
         ...
         "    source='Box Office Mojo (Worldwide) + TMDB genres',",
